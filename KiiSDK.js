...
root.KiiACLEntry = (function() {
  ...
  @param {KiiGroup|KiiUser|KiiAnyAuthenticatedUser|KiiAnonymousUser} subject instance.
  @throws {InvalidACLSubject} If the value is not one of the permitted values
   */

  KiiACLEntry.prototype.setSubject = function(value) {
    if ((value instanceof root.KiiGroup && (value.getID() != null)) || (value instanceof root.KiiUser && (value.getID() != null)) || value instanceof root.KiiAnyAuthenticatedUser || value instanceof root.KiiAnonymousUser) {
  ...
  @param {KiiGroup|KiiUser|KiiAnyAuthenticatedUser|KiiAnonymousUser} Subject
...
root.KiiEncryptedBucketAdmin = (function(_super) {
  __extends(KiiEncryptedBucketAdmin, _super);

  function KiiEncryptedBucketAdmin(bucketName, parent, token) {
    this.acl = __bind(this.acl, this);
    this.createObjectWithID = __bind(this.createObjectWithID, this);
    this.createObjectWithType = __bind(this.createObjectWithType, this);
    this.createObject = __bind(this.createObject, this);
    this._getRequest = __bind(this._getRequest, this);
    KiiEncryptedBucketAdmin.__super__.constructor.call(this, bucketName, parent);
    this._adminToken = token;
  }

  KiiEncryptedBucketAdmin.prototype._getRequest = function(spec) {
    var request;
    request = KiiEncryptedBucketAdmin.__super__._getRequest.call(this, spec);
    request.setAdminToken(this._adminToken);
    return request;
  };

  KiiEncryptedBucketAdmin.prototype.createObject = function() {
    return root.KiiObjectAdmin.objectWithBucket(this, null, this._adminToken);
  };

  KiiEncryptedBucketAdmin.prototype.createObjectWithType = function(type) {
    return root.KiiObjectAdmin.objectWithBucket(this, type, this._adminToken);
  };

  KiiEncryptedBucketAdmin.prototype.createObjectWithID = function(id) {
    return root.KiiObjectAdmin.objectWithID(this, id, this._adminToken);
  };

  KiiEncryptedBucketAdmin.prototype.acl = function() {
    var acl;
    acl = new root.KiiACLAdmin(this, this._adminToken);
    return acl;
  };

  return KiiEncryptedBucketAdmin;
  ...
  KiiObject.objectWithURI = function(uri) {
    ...
    if (compLength >= 4) {
      ...
      if (components[0] === "groups") {
        group = new root.KiiGroup.groupWithID(components[1]);
      } else if (components[0] === "users") {
        user = root.KiiUser.userWithID(components[1]);
      }
      subject = null;
      if (group != null) {
        subject = group;
...
root.KiiUser = (function() {
  ...
  @throws {InvalidArgumentException} If specified expiresAt is not a Date type.
  @example
  ...
  KiiUser.authenticateWithToken = function(token, callbacks, expiresAt) {
    ...
    if (expiresAtType !== 'undefined' && expiresAtType !== 'date') {
      throw new root.InvalidArgumentException('Specified expiresAt must be a Date');
    }
    user = new root.KiiUser();
  ...
  KiiUser.prototype.putIdentity = function(identityData, password, callbacks, userFields, removeFields) {
    var country, data, errorString, hasIdentityData, key, removeField, value, _i, _len;
    root.Kii.logger("Put identity user: " + this._uuid);
    data = {};
    if (!this.isPseudoUser()) {
      if (callbacks != null) {
        callbacks.failure(null, "This user has the identity already");
      }
      return;
    }
    if (identityData == null) {
      if (callbacks != null) {
        callbacks.failure(null, "identityData is null");
      }
      return;
    }
    try {
      ...
      errorString = _error;
      if (callbacks != null) {
        callbacks.failure(null, errorString);
      }
      return;
    }
    ...
    if (!hasIdentityData) {
      if (callbacks != null) {
        callbacks.failure(null, "needs to provide at least one of login name, email address or phone number");
      }
      return;
    }
    if (password != null) {
      if (!KiiUtilities._validatePassword(password)) {
        if (callbacks != null) {
          callbacks.failure(null, "invalid password");
        }
        return;
      }
    } else {
      if (callbacks != null) {
        callbacks.failure(null, "password is null");
      }
      return;
    }
    ...
    return this.refresh({
      failure: (function(_this) {
        return function(user, error) {
          root.Kii.logger("Failreg");
          if (callbacks != null) {
            return callbacks.failure(null, error);
          }
        };
      })(this),
      success: (function(_this) {
  ...
  callback.failure will be called.
  <br><br>
  <b>Note:</b>
  <ul>
  ...
  @throws {InvalidArgumentException}
      - If the email is null or empty<br>
      - If callback is null, or success/failure method does not exist.
  @throws {IllegalStateException}
      - If KiiUser is not logged in.
  @example
  KiiUser.findUserByEmail("user_to_find@example.com", {
      success: function(theFoundUser) {
  ...
  KiiUser.findUserByEmail = function(email, callbacks) {
    if ((root.KiiUser.getCurrentUser() == null) || (root.KiiUser.getCurrentUser().getAccessToken() == null)) {
      throw new root.IllegalStateException('User is not logged in');
    }
  ...
  KiiUser._findUserByEmail = function(email, callbacks, context) {
    var url;
    if ((email == null) || email === "") {
      throw new root.InvalidArgumentException('email should not null or empty');
    }
  ...
  @param {Method} callbacks.failure The callback method to call on a failed finding request.
  @throws {InvalidArgumentException}
      - If the phone is null or empty<br>
      - If callback is null, or success/failure method does not exist.
  @throws {IllegalStateException}
      - If KiiUser is not logged in.
  @example
  ...
  KiiUser._findUserByPhone = function(phone, callbacks, context) {
    var url;
    if ((phone == null) || phone === "") {
      throw new root.InvalidArgumentException('phone should not null or empty');
    }
  ...
  If there are no user registers with the specified user name, callback.failure will be called.
  <br><br>
  <b>Note:</b>
  <ul>
  <li>If "Expose Full User Data To Others" is enabled in the application console, the response will contain full of the user data.</li>
  ...
  @param {Method} callbacks.failure The callback method to call on a failed finding request.
  @throws {InvalidArgumentException}
      - If the username is null or empty.<br>
      - If callback is null, or success/failure method does not exist.
  @throws {IllegalStateException}
      - If KiiUser is not logged in.
  @example
  ...
  KiiUser._findUserByUsername = function(username, callbacks, context) {
    ...
    if ((username == null) || username === "") {
      throw new root.InvalidArgumentException('username should not null or empty');
    }
  ...
  KiiUser._findUserRequestImpl = function(url, callbacks, context) {
    ...
    if ((callbacks == null) || (callbacks.success == null) || (callbacks.failure == null)) {
      throw new root.InvalidArgumentException('Callback should not be null, and should exist success and failure methods.');
    }
    wrapper = KiiXHRWrapperFactory.createXHRWrapper("GET", url);
    xhr = wrapper.xhr;
    xhr.setRequestHeader("x-kii-appid", root.Kii.getAppID());
      ...
      success: function() {
        ...
        if (callbacks != null) {
          try {
            json = JSON.parse(xhr.responseText);
            if (context != null) {
              user = new root.KiiUserAdmin(context._getToken());
              user._updateWithJSON(json);
...
root.KiiAppAdminContext = (function() {
  ...
  KiiAppAdminContext.prototype.encryptedBucketWithName = function(bucketName) {
    var bucket;
    bucket = new root.KiiEncryptedBucketAdmin(bucketName, null, this._token);
    return bucket;
  };
  ...
  Kii.authenticateAsAppAdmin("client-id", "client-secret", {
      success: function(adminContext) {
          adminContext.findUserByEmail("user_to_find@example.com", {
              success: function(adminContext, theFoundUser) {
                  // Do something with the found user
              },
...
root.KiiGroupAdmin = (function(_super) {
  ...
  KiiGroupAdmin.prototype.encryptedBucketWithName = function(name) {
    var bucket;
    bucket = new root.KiiEncryptedBucketAdmin(name, this, this._adminToken);
    return bucket;
  };
...
root.KiiUserAdmin = (function(_super) {
  ...
  KiiUserAdmin.prototype._setAuthHeader = function(xhr) {
    return xhr.setRequestHeader("Authorization", "Bearer " + this._adminToken);
  };

  KiiUserAdmin.prototype.bucketWithName = function(bucketName) {
    var bucket;
    bucket = new root.KiiBucketAdmin(bucketName, this, this._adminToken);
    return bucket;
  };

  KiiUserAdmin.prototype.encryptedBucketWithName = function(bucketName) {
    var bucket;
    bucket = new root.KiiEncryptedBucketAdmin(bucketName, this, this._adminToken);
    return bucket;
  };
...
root.KiiSCNQQ = (function(_super) {
  ...
  KiiSCNQQ.prototype._register = function(options, callbacks) {
    var currentTime, now, registrationCallbacks, request, requestData;
    requestData = {
    ...
    request = new KiiRequest("/integration/qq", true);
    request.setMethod("POST");
    request.setData(requestData);
    request.setAnonymous(true);
    request.setContentType("application/vnd.kii.AuthTokenQQRequest+json");
    registrationCallbacks = {
      success: (function(_this) {
        return function(data) {
          var tokenObject, user, value;
          _this._setToken(options.access_token);
          tokenObject = {
  ...
  KiiSCNQQ.prototype._link = function(options, callbacks) {
    var data, linkCallbacks, request;
    request = new KiiRequest("/users/me/qq/link", true);
    request.setMethod("POST");
    data = {
      accessToken: options.access_token,
      openID: options.openID
  ...
  KiiSCNQQ.prototype._unlink = function(callbacks) {
    var request, unlinkCallbacks;
    request = new KiiRequest("/users/me/qq/unlink", true);
    request.setMethod("POST");
    unlinkCallbacks = {
      success: (function(_this) {
        return function(data) {
  ...
  KiiSCNQQ.prototype._logIn = function(options, callbacks) {
    KiiSCNQQ.__super__._logIn.call(this, options, callbacks);
    root.Kii.logger("Checking options");
    root.Kii.logger(options);
...
root.KiiServerCodeEntry = (function() {
  ...
  /**
   ...
   @throws {InvalidArgumentException} Thrown when argument is invalid in the following cases: <br>
   <li>argument is not type of Object </li>
   <li>argument is empty Object </li>
   @example
  ...
  KiiServerCodeEntry.prototype.execute = function(argument, callbacks) {
    ...
    if (!(KiiUtilities._validateServerCodeEntryArgument(argument))) {
      throw new root.InvalidArgumentException('arugment is invalid');
    }
      ...
      success: (function(_this) {
        return function() {
          ...
          try {
            ...
            if (callbacks != null) {
              return callbacks.success(_this.entryName, argument, execResult);
            }
          } catch (_error) {
            if (callbacks != null) {
              ...
              return callbacks.failure(_this.entryName, argument, null, errorString);
            }
          }
        };
      })(this),
      failure: (function(_this) {
        return function() {
          ...
          try {
            ...
            if (callbacks != null) {
              callbacks.failure(_this.entryName, argument, null, errorString);
            }
          }
        };
      })(this)
